...
export default function AdminKegiatanView({ 
  ...
  // Generate tahun options (dari tahun sekarang mundur 10 tahun)
  ...
  const handleSubmitKegiatan = async (e) => {
    ...
    try {
      if (!formData.nama || !formData.kategori || !formData.deskripsi || !formData.tahun) {
        setError('Nama, kategori, deskripsi, dan tahun wajib diisi');
        setIsLoading(false);
        return;
      }
      ...
      formDataToSend.append('tahun', formData.tahun.trim());

      if (selectedImage) {
        formDataToSend.append('foto', selectedImage);
      }

      const url = isEditing ? `/api/kegiatan/update` : '/api/kegiatan/add';
      ...
      if (result.success) {
        ...
        setError('');
        showAlert('success', isEditing ? 'Kegiatan berhasil diupdate!' : 'Kegiatan berhasil ditambahkan!');
        setTimeout(() => window.location.reload(), 1500);
      } else {
        setError(result.message || `Gagal ${isEditing ? 'mengupdate' : 'menambahkan'} kegiatan`);
      }

    } catch (error) {
      console.error(`Error saat ${isEditing ? 'update' : 'tambah'}:`, error);
      setError(error.message || `Terjadi kesalahan saat ${isEditing ? 'update' : 'tambah'} kegiatan`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleEdit = (item) => {
    ...
    setFormData({
      ...
      tahun: item.tahun ? item.tahun.toString() : ''
    });
    if (item.foto) {
  ...
  const handleDelete = async (itemId) => {
    showConfirmDialog(
      ...
      async () => {
        try {
          const response = await fetch(`/api/kegiatan/delete`, {
            method: 'DELETE',
            headers: {
          ...
          if (result.success) {
            ...
            showAlert('error', result.message || 'Failed to delete kegiatan');
          }
        } catch (error) {
          console.error('Error deleting kegiatan:', error);
          showAlert('error', 'An error occurred while deleting the kegiatan');
        }
      }
    );
  };
    ...
    return (
      <div className="flex items-center justify-between px-6 py-4 bg-gray-50 border-t">
        <div className="flex items-center text-sm text-gray-700">
          Showing {startIndex + 1} to {Math.min(endIndex, totalItems)} of {totalItems} kegiatan
        </div>
      ...
      <div className="bg-white shadow-lg rounded-lg overflow-hidden">
        ...
        <div className="p-6 border-b">
          <div className="flex justify-between items-center">
            <h2 className="text-2xl font-bold text-gray-800">Manajemen Data Kegiatan</h2>
            <button
              onClick={openModal}
          ...
          {data && data.length ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 text-lg">No kegiatan found</p>
              <p className="text-gray-400 text-sm">Click "Tambah Kegiatan Baru" to get started</p>
            </div>
          )}
        </div>
      </div>
                ...
                <form onSubmit={handleSubmitKegiatan} className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Masukkan deskripsi kegiatan secara detail..."
                      required
                    />
                  </div>
