...
import { X, ArrowRight, Calendar, Users, Music, Award, Flower, Gamepad2, Briefcase, Heart } from 'lucide-react';
              ...
              <div className="p-6 sm:p-8 md:p-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                  <div className="text-white text-opacity-70">
                    ...
                    <p className="text-lg font-semibold text-white">{item.tahun}</p>
                  </div>
                  <div className="text-white text-opacity-70">
                    <span className="text-sm font-medium">Kategori:</span>
                    <p className="text-lg font-semibold inline-block px-3 py-1 rounded-full mt-1 bg-white bg-opacity-20 text-white">
                      {item.kategori}
                    </p>
                  </div>
                </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mb-10">
            {currentItems.map((item) => {
              ...
              return (
                <div 
                  key={item.id} 
                  className="group bg-white-500 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden transform hover:-translate-y-2 hover:scale-105 border border-gray-100 cursor-pointer"
                ...
                >
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex justify-between items-center text-xs text-gray-500 mb-4 bg-gray-50 px-3 py-2 rounded-lg">
                      <span className="font-medium text-orange-500">Tahun: {item.tahun}</span>
                      <span className="px-2.5 py-1 rounded-full font-medium bg-green1-500 text-white-500">
                        {item.kategori}
                      </span>
                    </div>
          ...
          <div className="text-center bg-green1-500 rounded-xl p-4 shadow-lg border border-gray-100">
            <p className="text-sm text-white-500 font-medium">
              Menampilkan <span className="font-bold text-emerald-600">{startIndex + 1}-{Math.min(startIndex + itemsPerPage, filteredData.length)}</span> dari <span className="font-bold text-emerald-600">{filteredData.length}</span> kegiatan
