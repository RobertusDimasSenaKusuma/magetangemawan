...
import { X, ArrowRight, Building2, Users, Heart, Shield, Settings, MapPin, School, Briefcase } from 'lucide-react';
              ...
              <div className="p-6 sm:p-8 md:p-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                  <div className="text-white text-opacity-70">
                    ...
                    <p className="text-lg font-semibold text-white">{item.tahun_berdiri}</p>
                  </div>
                  {item.status && (
                    <div className="text-white text-opacity-70">
                ...
                {/* Kegiatan (if available) */}
                {item.kegiatan && (
                  <div className="p-4 sm:p-6 bg-white bg-opacity-10 rounded-xl">
                    <h4 className="font-semibold text-white mb-2 sm:mb-3 text-base sm:text-lg">Kegiatan:</h4>
                    <p className="text-white text-opacity-90 text-sm sm:text-base">{item.kegiatan}</p>
                  </div>
                )}
              </div>
            </motion.div>
...
export default function LembagaComponent() {
  ...
  if (!contentData || !contentData.lembaga || contentData.lembaga.length === 0) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
              ...
              return (
                ...
                >
                  <div className="relative h-56 overflow-hidden">
                    ...
                    {/* Maps Button for each item (if available) */}
                    {item.maps_link && (
                      <div className="absolute bottom-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <a
                          ...
                          className="w-10 h-10 bg-white hover:bg-red-600 rounded-full flex items-center justify-center shadow-lg transition-all duration-300 transform hover:scale-110"
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex justify-between items-center text-xs text-orange-500 mb-4 bg-gray-50 px-3 py-2 rounded-lg">
                      <span className="font-medium text-emerald-600">Tahun Berdiri: {item.tahun_berdiri}</span>
                    </div>
          ...
          <div className="text-center bg-green1-500 rounded-xl p-4 shadow-lg border border-gray-100">
            <p className="text-sm text-white-500 font-medium">
              Menampilkan <span className="font-bold text-emerald-600">{startIndex + 1}-{Math.min(startIndex + itemsPerPage, filteredData.length)}</span> dari <span className="font-bold text-emerald-600">{filteredData.length}</span> lembaga
